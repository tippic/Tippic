            ...
            if count>=1 or globle_excel_report_id.sheet_per_page:
                for field in globle_excel_report_id.field_ids:
                    ...
                    if field.field_id.ttype=="datetime":
                        record = datetime.strftime(pytz.utc.localize(datetime.strptime(record, DEFAULT_SERVER_DATETIME_FORMAT)).astimezone(local_tz),"%d/%m/%Y %H:%M:%S") 
                ...
                for i in globle_excel_report_id.field_ids:
                    if isinstance(record[i.field_id.name], tuple):
                        if str(i.field_id.ttype) == 'datetime':
                            record[i.field_id.name][1] = datetime.strftime(pytz.utc.localize(datetime.strptime(record[i.field_id.name][1], DEFAULT_SERVER_DATETIME_FORMAT)).astimezone(local_tz),"%d/%m/%Y %H:%M:%S")
                        worksheet.write(row,col, record[i.field_id.name][1])
                    else:
                        if str(i.field_id.ttype) == 'datetime':
                            record[i.field_id.name] = datetime.strftime(pytz.utc.localize(datetime.strptime(record[i.field_id.name], DEFAULT_SERVER_DATETIME_FORMAT)).astimezone(local_tz),"%d/%m/%Y %H:%M:%S")
        ...
        for record in self._context.get('active_ids'):
            ...
            for header_value in globle_excel_report_id.field_ids:
                ...
                if header_value.field_id.ttype=="datetime":
                        record_data = datetime.strftime(pytz.utc.localize(datetime.strptime(record_data, DEFAULT_SERVER_DATETIME_FORMAT)).astimezone(local_tz),"%d/%m/%Y %H:%M:%S") 
